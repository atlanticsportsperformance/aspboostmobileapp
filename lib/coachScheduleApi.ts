...
export function buildSessionsUrl(base: string, date: Date, tzOffset: number, categoryId?: string, mineOnly = true): string {
  let url = `${base}/api/schedule/sessions?date=${isoDate(date)}&tz_offset=${tzOffset}`;
  if (mineOnly) url += `&my_sessions=true`;
  if (categoryId) url += `&category_id=${categoryId}`;
  return url;
}

export async function getCoachTodaysSessions(date: Date, categoryId?: string, mineOnly = true): Promise<CoachSession[]> {
  const { data: { session } } = await supabase.auth.getSession();
  if (!session?.access_token) throw new Error('Not authenticated');
  const tzOffset = new Date().getTimezoneOffset();
  const url = buildSessionsUrl(API_URL, date, tzOffset, categoryId, mineOnly);
