...
export default function LoginScreen({ navigation }: any) {
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(pulse1, { toValue: 0.5, duration: 4000, useNativeDriver: true }),
        Animated.timing(pulse1, { toValue: 0.3, duration: 4000, useNativeDriver: true }),
      ])
    ).start();

    Animated.loop(
      Animated.sequence([
        Animated.timing(pulse2, { toValue: 0.4, duration: 5000, useNativeDriver: true }),
        Animated.timing(pulse2, { toValue: 0.2, duration: 5000, useNativeDriver: true }),
      ])
    ).start();
  }, []);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.contentContainer}>
          <View style={styles.logoContainer}>
            <LinearGradient colors={['#9BDDFF', '#B0E5FF', '#7BC5F0']} style={styles.logo}>
              <Text style={styles.logoText}>A</Text>
            </LinearGradient>
          </View>

          <View style={styles.titleContainer}>
            <Text style={styles.title}>ASP <Text style={styles.titleGradient}>Boost+</Text></Text>
          ...
          <View style={styles.card}>
            ...
            <TouchableOpacity onPress={() => setRememberMe(!rememberMe)} style={styles.rememberMeContainer} activeOpacity={0.7} disabled={loading}>
            ...
            {biometricAvailable && !hasSavedCredentials && (
              <TouchableOpacity onPress={() => setEnableFaceId(!enableFaceId)} style={styles.rememberMeContainer} activeOpacity={0.7} disabled={loading}>
                <View style={[styles.checkbox, enableFaceId && styles.checkboxChecked]}>
                  {enableFaceId && <Text style={styles.checkmark}>✓</Text>}
                </View>
                <Text style={styles.rememberMeText}>Enable {Platform.OS === 'ios' ? 'Face ID' : 'Biometric'} sign in</Text>
              </TouchableOpacity>
            )}

            <TouchableOpacity onPress={handleLogin} disabled={loading} style={[styles.button, loading && styles.buttonDisabled]} activeOpacity={0.9}>
              <LinearGradient colors={['#9BDDFF', '#7BC5F0']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.buttonGradient}>
...
const styles = StyleSheet.create({
  ...
  contentContainer: { paddingHorizontal: 16, maxWidth: 448, width: '100%', alignSelf: 'center' },
  logoContainer: { alignItems: 'center', marginBottom: 24 },
  logo: { width: 64, height: 64, borderRadius: 16, alignItems: 'center', justifyContent: 'center', shadowColor: '#9BDDFF', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 10 },
  logoText: { fontSize: 28, fontWeight: 'bold', color: '#000000' },
  titleContainer: { alignItems: 'center', marginBottom: 32 },
  title: { fontSize: 40, fontWeight: 'bold', color: '#FFFFFF', marginBottom: 8, letterSpacing: -0.5 },
  titleGradient: { color: '#9BDDFF' },
  subtitle: { fontSize: 14, color: 'rgba(255, 255, 255, 0.5)', textAlign: 'center' },
  card: { position: 'relative', backgroundColor: 'rgba(15, 20, 25, 0.95)', borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.1)', borderRadius: 24, padding: 32, shadowColor: '#000', shadowOffset: { width: 0, height: 20 }, shadowOpacity: 0.5, shadowRadius: 30, elevation: 20 },
  welcomeContainer: { alignItems: 'center', marginBottom: 32 },
  welcomeTitle: { fontSize: 24, fontWeight: 'bold', color: '#FFFFFF', marginBottom: 4 },
  welcomeSubtitle: { fontSize: 14, color: 'rgba(255, 255, 255, 0.6)', textAlign: 'center' },
  errorContainer: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: 'rgba(239, 68, 68, 0.1)', borderWidth: 1, borderColor: 'rgba(239, 68, 68, 0.2)', borderRadius: 16, padding: 16, marginBottom: 24 },
  errorIcon: { width: 20, height: 20, borderRadius: 10, backgroundColor: 'rgba(239, 68, 68, 0.2)', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  errorText: { flex: 1, color: '#EF4444', fontSize: 14, fontWeight: '500' },
  inputGroup: { marginBottom: 24 },
  label: { fontSize: 14, fontWeight: '500', color: 'rgba(255, 255, 255, 0.9)', marginBottom: 8 },
  labelRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  forgotPassword: { fontSize: 12, color: '#9BDDFF' },
  ...
  faceIdInlineButton: { padding: 8, marginLeft: 8 },
  rememberMeContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: 24 },
  checkbox: { width: 16, height: 16, borderRadius: 4, borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.1)', backgroundColor: 'rgba(255, 255, 255, 0.05)', marginRight: 8, alignItems: 'center', justifyContent: 'center' },
  checkboxChecked: { backgroundColor: '#9BDDFF', borderColor: '#9BDDFF' },
  checkmark: { color: '#000', fontSize: 12, fontWeight: 'bold' },
  rememberMeText: { fontSize: 14, color: 'rgba(255, 255, 255, 0.7)' },
  button: { borderRadius: 12, overflow: 'hidden', shadowColor: '#9BDDFF', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 8, elevation: 8 },
  buttonDisabled: { opacity: 0.5 },
  buttonGradient: { paddingVertical: 14 },
  buttonContent: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  buttonText: { fontSize: 16, fontWeight: '600', color: '#000000' },
  spinner: { fontSize: 20, color: '#000' },
  security: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 32, gap: 8 },
  securityText: { fontSize: 12, color: 'rgba(255, 255, 255, 0.4)', marginLeft: 8 },
  footer: { textAlign: 'center', fontSize: 12, color: 'rgba(255, 255, 255, 0.3)', marginTop: 24 },
  newHereContainer: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', marginTop: 24 },
  newHereText: { fontSize: 14, color: 'rgba(255, 255, 255, 0.6)' },
  newHereLink: { fontSize: 14, fontWeight: '600', color: '#9BDDFF' },
});
