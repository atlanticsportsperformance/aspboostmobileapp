...
describe('the memberships tab is wired to the helper, not to hardcoded plans', () => {
  ...
  it('opens the existing purchase flow from both the featured CTA and a compact row', () => {
    const finder = SOURCE.slice(SOURCE.indexOf('function renderPlanFinder'));
    expect(finder.match(/openMembershipPurchase\(/g)?.length).toBeGreaterThanOrEqual(2);
  });
